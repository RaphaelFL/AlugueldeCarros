...
public class ReservationsControllerTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    [Fact]
    public async Task GetById_WithOwnerToken_ReturnsReservation()
    {
        using var client = _factory.CreateApiClient();
        ...
        document.RootElement.GetProperty("userId").GetInt32().Should().Be(1);
    }
    ...
    public async Task GetById_WithDifferentCustomerToken_ReturnsForbidden()
    {
        ...
        var response = await client.GetAsync("/api/v1/reservations/2");
